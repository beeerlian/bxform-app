...
import { Message } from 'utils/email/email.interfaces'
import logger from 'utils/logger'
import { htmlChangePhone } from './email.html.changed-phone'
...
/**
 * Send email to verfiy change of mobile number
 * @param {string} to
 * @param {string} token
 * @param {string} name
 * @returns {Promise<void>}
 *  */
export const sendMobileVerificationEmail = async (to: string, token: string, expireAt: string): Promise<void> => {
  const subject = 'Verifikasi Nomor Handphone'

  const verificationEmailUrl = `${config.clientUrl}/change_phone?token=${token}&expireAt=${expireAt}`

  var template = handlebars.compile(htmlChangePhone)
  ...
  sendEmail(to, subject, '', htmlToSend)
    .then(() => {
      logger.info(`Send mail change mail successfully to ${to}`)
    })
    .catch(err => {
      logger.error(err)
    })
